...
def create_plot(layout):
    ...
    nodes.selection_glyph = Circle(size=15, fill_color=Plasma11[10], line_color='colors', fill_alpha=0.8)
    nodes.hover_glyph = Circle(size=15, fill_color=Plasma11[9],line_color='colors')
...
def create_tabs():
    plot1=create_plot(layout="grouped")
    tab1 = Panel(child=plot1, title="Grouped by Category")
    plot2=create_plot(nx.kamada_kawai_layout)
    tab2 = Panel(child=plot2, title="Kamada Kawai Layout")
    plot3=create_plot(nx.circular_layout)
    tab3 = Panel(child=plot3, title="Circular Layout")
    plot2=create_plot(nx.spiral_layout)
